...
double ran2(long *idum)	//zufallszahlengenerator ran2 nach numerical recipes for c
{
    int j;
    long k;
    ...
    {
        if (-(*idum) < 1)
            *idum=1;				// verhindere idum=1
        else
            *idum = -(*idum);
        
        idum2 = (*idum);
        
        for (j=NTAB+7;j>=0;j--)					// f¸lle shuffle table nach 8 durchl‰ufen
        {
            k = (*idum)/IQ1;
